...
import './GenerateTopicReportCardSupervisorStyle.css';
import { useNavigate } from 'react-router-dom';
import config from '../../../config';
import { getUserId } from '../../../getUserId';

export const GenerateTopicReportCardSupervisor = ({ open, handleClose }) => {
  ...
  const handleGeneratePDFFile = async () => {
    try {
      ...
      if (!generatePDFResponse.ok) {
        if (generatePDFResponse.status === 401) { 
          console.error('Błąd autoryzacji: ', generatePDFResponse.status);
  ...
  return (
    ...
    >
      <Box className="generate-topic-form-modal-supervisor">
        <IconButton
          aria-label="close"
          onClick={handleClose}
        ...
        <form className="topic-card-registration-supervisor">
          ...
          />
            ...
            <label htmlFor="academic-department-select-supervisor">Wydział:</label>
            <select
              className="academic-department-select-supervisor"
          ...
          <label htmlFor="academic-department-select-supervisor">Katedra:</label>
          <select
            className="academic-department-select-supervisor"
          ...
          <TextareaAutosize
            className="textarea-properties-generate-supervisor"
            minRows={5}
          ...
          <label htmlFor="subject-select-supervisor">Kierunek:</label>
          <select
            className="subject-select-supervisor"
